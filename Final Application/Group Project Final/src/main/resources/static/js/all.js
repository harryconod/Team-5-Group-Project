...
let generateShop = (data) => {
    shop.innerHTML = data.map((x) => {
        ...
        // Fetch the img data from the API
        fetch(`/api/products/${id}`)
          .then((response) => response.blob())
          .then((blob) => {
            // Convert the img data to base64 format
            const reader = new FileReader();
            reader.readAsDataURL(blob);
            reader.onloadend = () => {
              
              // Assign the img tag with the base64 img data
            // Assume the base64 img data is stored in the `base64data` variable
                //let imageElement = `<img class="img-fluid" src="${img}" alt="" >`;
                let imageElement=`<img class="img-fluid"  src="${img}" alt="" onclick="showImageDetails('${name}', '${desc}', '${category}', '${img}','${price}')">`;
...
function showImageDetails(name, desc, category, img, price) {
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="img-modal-label">${name}</h5>
    ...
    document.querySelector('#img-modal .close').addEventListener('click', () => {
      ...
      document.body.removeChild(document.getElementById('img-modal'));
    });
  } 
